...
def plot_piechart(x, labels, title="", fig_size=(10, 5), save=None):
    ...
    annots = [
        "{} - {:.2f}% ({:d})".format(c, p, n) for c, p, n in zip(labels, percents, x)
    ]
...
def plot_confusion_metrix(
    y_true, y_pred, labels=None, title="", normalize=None, fig_size=(10, 10), save=None
):
