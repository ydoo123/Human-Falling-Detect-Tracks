...
def transformBoxInvert_batch(pt, ul, br, inpH, inpW, resH, resW):
    ...
    _pt[:, :, 0] = _pt[:, :, 0] - (
        (lenW[:, np.newaxis].repeat(1, num_pt) - 1) / 2
        - center[:, 0].unsqueeze(-1).repeat(1, num_pt)
    ).clamp(min=0)
    _pt[:, :, 1] = _pt[:, :, 1] - (
        (lenH[:, np.newaxis].repeat(1, num_pt) - 1) / 2
        - center[:, 1].unsqueeze(-1).repeat(1, num_pt)
    ).clamp(min=0)
...
def crop_dets(img, boxes, height, width):
    ...
    for i, box in enumerate(boxes):
        ...
        bottomRight[0] = max(
            min(img_w - 1, bottomRight[0] + w * scaleRate / 2), upLeft[0] + 5
        )
        bottomRight[1] = max(
            min(img_h - 1, bottomRight[1] + h * scaleRate / 2), upLeft[1] + 5
        )
