...
class Models:
    ...
    def load_models(self):
        ...
        self.pose_model = SPPE_FastPose(
            self.pose_backbone, self.inp_pose[0], self.inp_pose[1], device=self.device
        )
    ...
    def process_frame(self, frame):
        ...
        for track in self.tracker.tracks:
            det = torch.tensor(
                [track.to_tlbr().tolist() + [1.0, 1.0, 0.0]], dtype=torch.float32
            )
            detected = (
...
class main:
    def __init__(self, master: tk.Tk):
        ...
        self.width = int(self.main_screen.width * 0.85)
        self.height = int(self.main_screen.height * 0.85)
        self.master.geometry("{}x{}".format(self.width, self.height + 15))

        self.cam = None
        self.canvas = tk.Canvas(
            master, width=int(self.width * 0.65), height=self.height
        )
    ...
    def load_cam(self, source):
        ...
        if type(source) is str and os.path.isfile(source):
            self.cam = CamLoader_Q(
                source, queue_size=1000, preprocess=self.preproc
            ).start()
        else:
